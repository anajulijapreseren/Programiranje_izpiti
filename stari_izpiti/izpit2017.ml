...
(* 1.1) Definirajte funkcijo, ki vzame tri cela števila ter vrne njihov produkt.
   ...
   (* 1.3) Definirajte funkcijo, ki vzame seznam in izračuna seznam vrednosti funkcije
    ...
    let vse_kubiraj_in_pristej_dva sez= List.map (fun  x -> afin_kub x  2) sez
   ...
   (* 1.4) Definirajte funkcijo, ki varno vrne zadnji element seznama v primeru,
    ...
    let zadnji_element sez = match List.rev sez with
      | [] -> None
      | x::xs -> Some x
   ...
   let rec vsa_pozitivna (Rose (n, forest))= 
    if n<=0 then false
    else not (List.mem false (List.map vsa_pozitivna forest))
   ...
   (* 2.7) Definirajte funkcijo, ki pretvori rožno drevo v seznam. Vrstni red vrednosti v seznamu
    ...
    (*TO DO,ŠE NI OK*)
   let drevo_v_seznam (Rose (n, forest))= 
      let rec trenutno acc drevo = match drevo with
        |(Rose (n, [])) -> n::acc
        |(Rose (n, forest)) -> List.fold_left trenutno [n] forest
      in trenutno [] (Rose (n, forest))